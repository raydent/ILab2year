...
bool operator == (const page& c1, const page& c2) {//const &
...
page* intArrToPageArr(int* arr, int arrLen);//может стоит использовать ссылки?
...
int main() {
	...
	std::cin >> arrLen;
	int numArr[arrLen];// ???? массив задается не константным значением, это даже не скомпилируется на некоторых компиляторах, в c++ есть std::vector
