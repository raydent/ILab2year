...
struct polygon_t {
    ...
    polygon_t(){}
    explicit polygon_t(vertex2D_t<T> A_, vertex2D_t<T> B_, vertex2D_t<T> C_) {list.push_front(A_); list.push_front(B_); list.push_front(C_);}
    void push(int position, vertex2D_t<T>& elem);
    const double calc_square();
    const void print();
};
...
struct vec2D_t {
    ...
    vertex2D_t<T> A, B;
    double k;
    explicit vec2D_t(vertex2D_t<T> A_, vertex2D_t<T> B_) : x(B_.x - A_.x), y(B_.y - A_.y), A(A_), B(B_) {x == 0 ? k = 0 : k = y / x;}
    const double cross_product(vec2D_t<T> rhs);
    const double cross_product(vertex2D_t<T> rhs);
    const bool findCrossing(vec2D_t<T> rhs, vertex2D_t<T>& ret1);
    const void clipPolygon(vertex2D_t<T> C, polygon_t<T>& polygon);
    vec2D_t<T> makeVec(vertex2D_t<T> A_, vertex2D_t<T> B_);
    const bool rightside(vertex2D_t<T> C, vertex2D_t<T> rhs);
    const void remove_wrong_points_from_polygon(polygon_t<T>& polygon, vertex2D_t<T> C);
    const void print();
};
...
struct triangle_t {
    vertex2D_t<T> A, B, C;
    vec2D_t<T> AB, BC, CA;
    explicit triangle_t(vertex2D_t<T> A_, vertex2D_t<T> B_, vertex2D_t<T> C_): A(A_), B(B_), C(C_), AB{A_, B_}, BC{B_, C_}, CA{C_, A_} {}
    const double intersection_area(const triangle_t<T> &rhs);
    const void print();
};
...
template <typename T>
const double vec2D_t<T>::cross_product(vec2D_t<T> rhs){
    return ((y * rhs.x) - (x * rhs.y));
}

template <typename T>
const double vec2D_t<T>::cross_product(vertex2D_t<T> rhs){
    vec2D_t BC {B, rhs};
    return cross_product(BC);
}

template <typename T>
const void vec2D_t<T>::clipPolygon(vertex2D_t<T> C, polygon_t<T>& polygon){
...
template <typename T>
const double triangle_t<T>::intersection_area(const triangle_t<T> &rhs){
...
template <typename T>
const bool vec2D_t<T>::rightside(vertex2D_t<T> C, vertex2D_t<T> rhs){
...
template <typename T>
const bool vec2D_t<T>::findCrossing(vec2D_t<T> rhs, vertex2D_t<T>& ret1){
...
template <typename T>
const void vec2D_t<T>::print(){
    std::cout << x << " " << y << " " << A.x << " " << A.y << " " << B.x << " " << B.y << " " << k << "\n";
}

template <typename T>
const void triangle_t<T>::print(){
    std::cout << A.x << " " << A.y << " " << B.x << " " << B.y << " " << C.x << " " << C.y << " " << AB.x << " " << AB.y << " " << AB.k << "\n";
}

template <typename T>
const double polygon_t<T>::calc_square(){
    double square = 0;
    auto it1 = list.begin();
    auto it2 = list.begin();
    it2++;
...
template <typename T>
const void vec2D_t<T>::remove_wrong_points_from_polygon(polygon_t<T>& polygon, vertex2D_t<T> C){
